...
export async function fetchCars(page: number, elementPerPage: number, hidden: number, solded: number, brand: string, model: string): Promise<Array<CarProps>> {
    brand = '%'.concat (brand, '%')
    model = '%'.concat (model, '%')
    const query = 'SELECT * FROM cars WHERE solded = ? AND hidden = ? AND brand LIKE ? AND model LIKE ? LIMIT ? OFFSET ?'

    const db = await AutobensDatabase.getInstance().getDatabase()
    const cars = await db.all(query, [solded, hidden, brand, model, elementPerPage, elementPerPage*(page-1)])
    return cars
}
